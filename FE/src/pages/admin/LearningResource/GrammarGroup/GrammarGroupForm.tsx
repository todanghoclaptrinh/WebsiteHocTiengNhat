...
import { CreateUpdateGrammarGroupDTO } from '../../../../interfaces/Admin/GrammarGroup';

const GrammarGroupEditor: React.FC = () => {
  ...
  const [formData, setFormData] = useState<CreateUpdateGrammarGroupDTO>({
    groupName: '',
    description: ''
  });
  ...
  useEffect(() => {
    const loadTopicDetail = async () => {
        if (isEditMode && id) {
            try {
                const data = await grammarGroupService.getById(id);
                setFormData({
...
const handleSave = async () => {
    if (!formData.groupName.trim()) {
        alert("Vui lòng nhập tên nhóm ngữ pháp!");
        return;
    }

    // Tạo payload chuẩn theo CreateUpdateTopicDTO
    const payload: CreateUpdateGrammarGroupDTO = {
    ...
    try {
        setLoading(true);
        if (isEditMode && id) {
            await grammarGroupService.update(id, payload);
            alert("Cập nhật nhóm ngữ pháp thành công!");
        } else {
            await grammarGroupService.create(payload);
            alert("Thêm mới nhóm ngữ pháp thành công!");
        }
        navigate("/admin/resource/grammar-group");
    } catch (error: any) {
          ...
          <div className={isEditMode ? 'flex items-center w-full gap-230' : 'flex items-center w-full gap-226'}>
            <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex flex-col text-left">
                    <h2 className="text-xl font-bold text-[#181114] uppercase">
                        {isEditMode ? 'Chỉnh sửa nhóm ngữ pháp' : 'Thêm nhóm ngữ pháp'}
                    </h2>
            ...
            <div className="flex items-center gap-3">
                ...
                >
                    ...
                    {isEditMode ? 'Cập nhật' : 'Lưu nhóm ngữ pháp'}
                </button>
            </div>
        </div>
      </AdminHeader>
                ...
                <div className="flex flex-row gap-10">
                    ...
                    <div className="w-[40%] space-y-2">
                        ...
                        <input
                            ...
                            placeholder="e.g., Điều kiện, Khả năng..."
                            type="text"
                            value={formData.groupName}
                            onChange={(e) => setFormData({ ...formData, groupName: e.target.value })}
                        />
                        <p className="text-[12px] text-[#b399a4] italic">Tên nhóm ngữ pháp nên ngắn gọn, dễ hiểu.</p>
                    </div>

                    {/* Cột Mô tả nhóm ngữ pháp (60%) */}
                    <div className="w-[60%] space-y-2">
                        ...
                        <textarea
                            ...
                            placeholder="Mô tả mục tiêu hoặc nội dung của nhóm ngữ pháp này..."
...
export default GrammarGroupEditor;