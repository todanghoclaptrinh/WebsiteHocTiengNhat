...
import { ReadingItem } from '../../../../interfaces/Admin/Reading';
...
const ReadingManagement: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      setLoading(true);
      const [readingData, topicData] = await Promise.all([
        readingService.getAll(),
  ...
  const filteredData = passages.filter((item: any) => {
