...
const TopicManagement: React.FC = () => {
  ...
  const [topics, setTopics] = useState<TopicItem[]>([]); // Danh sách hiển thị chính
  ...
  const fetchTopics = async () => {
    try {
        setLoading(true);
        // Gọi trực tiếp getAll của topicService
  ...
  const confirmDelete = async () => {
    ...
    try {
        ...
        // Cập nhật lại danh sách topics sau khi xóa
        setTopics(topics.filter(t => t.topicID !== deleteId));
        setDeleteId(null);
        setTopicToDelete(null);
    } catch (error) {
        console.error(error);
        alert("Xóa thất bại! Chủ đề này có thể đang được sử dụng ở bài học khác.");
    }
  };
  ...
  const filteredData = topics.filter((item: TopicItem) => {
  ...
  return (
    <div className="flex flex-col h-full bg-background-light">
      <AdminHeader>
        <div className="flex items-center gap-204">
          <div className="flex items-center gap-4 flex-1">
            <div className="flex flex-col">
              <h2 className="text-xl font-bold text-[#181114]">QUẢN LÝ CHỦ ĐỀ</h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link 
              to="/admin/resource/topic/create"
      ...
      {deleteId && (
        <div className="fixed inset-0 z-999 flex items-center justify-center p-6">
            ...
            <div className="text-center mb-10">
                <h3 className="text-[24px] font-black text-[#181114] mb-3 tracking-tight">Xóa chủ đề?</h3>
                <p className="text-[#886373] text-sm leading-relaxed px-4">
                Chủ đề <span className="inline-block mt-2 font-bold text-[#e11d48] bg-[#fff1f2] px-3 py-1 rounded-xl italic">
      ...
      <div className="flex-1 overflow-hidden p-8">
        <div className="bg-white rounded-2xl border border-[#f4f0f2] shadow-sm overflow-hidden flex flex-col h-full">
            ...
            <table className="w-full text-left border-collapse table-fixed">
                ...
                {loading ? (
                    <tr>
                    <td colSpan={3} className="text-center py-10 text-sm text-slate-400">Đang tải danh sách chủ đề...</td>
                    </tr>
                ) : currentItems.length === 0 ? (
                    <tr>
                    <td colSpan={3} className="text-center py-10 text-sm text-slate-400">Không tìm thấy chủ đề nào.</td>
                    </tr>
                ) : currentItems.map((item) => (
                    <tr key={item.topicID} className="hover:bg-primary/5 transition-colors h-24.5">
                    ...
                    <td className="px-8 py-5 text-sm text-[#181114]">
                        ...
                        <div className="flex flex-col gap-0.5 overflow-hidden">
                            <span className="font-bold text-[15px] text-[#181114] truncate">{item.topicName}</span>
                    ...
                    <td className="px-8 py-5 text-right">
                        ...
                        <Link 
                            to={`/admin/resource/topic/edit/${item.topicID}`} 
                        ...
                        <button 
                            onClick={() => handleDelete(item.topicID, item.topicName)} 
            ...
            <p className="text-xs text-[#886373] font-medium">
                Hiển thị <span className="text-[#181114]">{indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredData.length)}</span> của {filteredData.length} chủ đề
            </p>
